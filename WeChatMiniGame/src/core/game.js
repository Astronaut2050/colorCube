...
export function createGame(options = {}) {
  ...
  function tick(timestamp) {
    ...
    if (spawnHoldPending) {
      spawnHoldPending = false;
      spawnHoldUntil = timestamp + SPAWN_MORPH_HOLD_MS;
    }

    if (currentPiece && timestamp < spawnHoldUntil) {
      fallProgress = 0;
      lastDropTime = timestamp;
      return;
    }
